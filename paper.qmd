...
#| fig.width: 12
#| fig.height: 6
#| fig.cap: Coefficient plot of predictors of immune investment (WBCC) from generalized linear models (Pilot). Variables labeled (S) have been centered at the mean and standardized by 2 SD. Bars are 95% CIs.

# plot_immune_both <- 
#   plot_immune + guides(colour = 'none', shape = 'none') + plot_immune_stage2 + plot_layout(axes = 'collect', guides = 'collect')
...
# p <- plot_energy_coefs / plot_protein_coefs + plot_layout(guides = 'collect', axis_titles = 'collect')
...
Models and R code for three of our four mating success outcome variables remain identical to those used in the Stage 1 pilot study. However, although the Poisson regression model of lifetime sexual partners in the pilot study included age as a control, it is more appropriate to instead include age as an exposure variable (an offset) in the model, which we now do. We also treat this exposure as years since sexual maturity (age - 12 years). The changes in the strength and strength:sex coefficients from this change in model specification are minimal (see @sfig-comparison).

We also discovered that because we included dietary energy intake in the expanded controls immune model we needed to utilize the weights calculated for the dietary data rather than the weights calculated for other data for this model. This is corrected in the confirmatory results for that model.
...
#| fig-width: 10
#| fig-height: 8
#| fig-cap: Effects of sex-specific standardized strength on mating outcomes, by sex. Strength is centered and scaled by 2 SD, separately for each sex (almost all participants therefore fall between -1 and 1). Plots are based on the anthropometric control models, with control variables held at their mean values. Lifetime partners is modeled as the rate of new partners per year since reaching sexual maturity (age 12).

plot_effects_all
```
Our confirmatory results revealed strong positive significant main effects of sex-specific strength on partnered status, numbers of lifetime partners after controlling for all five sets of potential confounds. There was no significant effect of sex-specific strength on the number of partners in the past year, and there were inconsistent effects of sex-specific strength on age at first sex across the potential confound models. 

Unlike findings from the pilot study, a clear sex difference in the effects of sex-specific strength only emerged for one mating success outcome---partnered status---with smaller effects for women compared to men, as predicted by the sexual selection hypothesis. The interaction between sex and sex-specific strength on lifetime number of sexual partners was not replicated in the confirmatory data.  Finally there was no evidence for a sex difference in the effects of sex-specific strength on past year number of partners or age at first sex, contrary to the sexual selection hypothesis.

Effects of sex-specific standardized strength on mating outcomes, by sex. Strength is centered and scaled by 2 SD, separately for each sex (almost all participants therefore fall between -1 and 1). Plots are based on the anthropometric control models, with control variables held at their mean values. Lifetime partners is modeled as the rate of new partners per year since reaching sexual maturity (age 12).

## Discussion
The purpose of this study was to replicate as closely as possible, the results from @lassek2009, in new data that included women and used combined grip strength as a proxy for upper body strength. We found consistent support for a positive relationship between combined grip strength and two measures of mating success: being currently partnered, and lifetime number of sexual partners for both men and women, controlling for numerous possible confounds. We found no consistent association between strength and age at first sex or number of past year sexual partners. 

We also tested for interactions between sex and strength on these mating success outcomes. Across both pilot and confirmatory data, we found a consistent interaction between sex and strength on partnered status, such that grip strength was more important for predicting males' partnered status than females. Although in the pilot data we did see evidence of an interaction between sex and strength on the outcome lifetime number of sexual partners, such that strength was more important for predicting males' lifetime sexual partners, this effect was not present in the confirmatory data. 

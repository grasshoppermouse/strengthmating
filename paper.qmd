...
```{r}
#| label: fig-allcoefs
#| fig-cap: Coefficients from all models of our four mating success outcomes and our five sets of potential confounding variables. Partnered coefficients are from logistic regressions, partner number coefficients are from quasi-Poisson glm regressions, and age at first sex coefficients are from Gaussian glm regressions. Bars are 95\% CIs. For the full regression tables, see the SI.
...
#| fig.width: 12
#| fig.height: 8
#| fig.cap: Coefficient plot of predictors of dietary energy intake (kcals) and dietary protein (g) from generalized linear models (Pilot). Variables labelled (S) have been centered at the mean and standardized by 2 SD. Bars are 95% CIs.
...
p <- plot_energy + plot_protein + plot_layout(axes = 'collect') 
ggsave("Figures/plot_energy_protein.pdf", p, width = 12, height = 6)
p
```
...
Models and R code for three of our four mating success outcome variables remain identical to those used in the Stage 1 pilot study. However, although the Poisson regression model of lifetime sexual partners in the pilot study included age as a control, it is more appropriate to instead include it as an exposure variable (an offset) in the model, which we now do. We also treat this exposure as years since sexual maturity (age - 12 years). The change in the strength and strength:sex coefficients from this change in model specification is minimal (see @sfig-comparison).
...
cor_bothsexes <- ggcorrplot(cor_mat_cors, lab = T, hc.order = T, hc.method = 'ward.D', title = 'Both sexes')
...
cor_females <- ggcorrplot(cor_mat_cors_f, lab = T, hc.order = T, hc.method = 'ward.D', title = 'Females')
...
cor_males <- ggcorrplot(cor_mat_cors_m, lab = T, hc.order = T, hc.method = 'ward.D', title = 'Males')
...
for (i in 1:nrow(modelsummaries)){
  cat(paste('####', modelsummaries$title[i]))
...
for (i in 1:nrow(dfmodelsummaries)){
  cat(paste('####', dfmodelsummaries$title[i]))

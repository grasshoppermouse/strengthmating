...
Table one shows weighted means and standard deviations of all variables for men and women ages 18-59. Mean age was 38 years for this sample. Combined grip strength, height, weight, and testosterone were significantly higher in males than females.  while BMI was significantly higher in females than males (a reversal from Lassek and Gaulin). 
...
# has all!
tabb3a <- tabb2a %>% modify_table_body(
  ~ .x %>%
    dplyr::mutate(d = round(tabba$table_body$estimate, digits = 2))) %>%
  modify_header(d = "**d**") %>%
  modify_header(label ~ "**Variable**") %>%
  modify_spanning_header(c("ratio", "d") ~ "**Sexual dimorphism**") %>%
  modify_caption("**Table 1. Sex differences based on Lassek and Gaulin Table 1 for ages 18-49**")
...
We did not include an interaction between CGS and partnered status for models of age at first sex since, for the majority of participants their age at first sex was in the past and unlikely to be predicted by current grip strength or current partnered status. 
